...
export default function Ingenieria() {
  ...
  // Cálculo dinámico de filas que entran según el alto del monitor
  useEffect(() => {
  ...
  useEffect(() => {
    if (!selectedItem) {
      setIngenierias([]);
      setActiveIngenieria(null);
      return;
    }
    const fetchIngenierias = async () => {
      try {
        const endpoint =
          mainTab === "SE" ? "semielaborado" : "producto-terminado";
        const res = await fetch(
          `http://localhost:3001/api/ingenierias/${endpoint}/${selectedItem.id}`,
        );
        const data = await res.json();
        setIngenierias(data);
        setActiveIngenieria(
          data.find((i) => i.es_activa === 1) || data[0] || null,
        );
        setIsDropdownOpen(false);
      } catch (err) {
  ...
  const currentPaginatedItems = filteredDataset.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  ...
  const handleSetActiva = async (ingId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/ingenierias/${ingId}/activar`,
        {
          method: "PUT",
      ...
      if (res.ok) {
        const updated = ingenierias.map((i) => ({
          ...i,
          es_activa: i.id === ingId ? 1 : 0,
        }));
        setIngenierias(updated);
        setActiveIngenieria(updated.find((i) => i.id === ingId));
      }
    } catch (err) {
      console.error("Error activando receta:", err);
    }
  };
  ...
  const handleOpenCreateModal = () => {
    ...
    setRecipeForm({
      id: null,
      nombre_version: `Receta v${ingenierias.length + 1}`,
      es_activa: ingenierias.length === 0,
      ingredientes: [],
    });
    setMpFilterTerm("");
    setMpCurrentPage(1);
  ...
  const handleOpenEditModal = (recipe) => {
    setRecipeForm({
      id: recipe.id,
      nombre_version: recipe.nombre_version,
      es_activa: recipe.es_activa === 1,
  ...
  const handleMpFilterChange = (e) => {
    setMpFilterTerm(e.target.value);
    setMpCurrentPage(1);
  };
  ...
  const handleAddIngredientRow = (item) => {
    const exists = recipeForm.ingredientes.some(
      (i) =>
        (item.type === "MP" && i.materia_prima_id === item.id) ||
        (item.type === "SE" && i.semielaborado_id === item.id),
    );
  ...
  const handleUpdateIngredientQty = (tempId, qty) => {
    setRecipeForm((prev) => ({
      ...
      ingredientes: prev.ingredientes.map((i) =>
        i.id_temp === tempId || (i.item_codigo && i.item_codigo === tempId)
          ? { ...i, cantidad: parseFloat(qty) || 0 }
          : i,
      ),
    }));
  };
  ...
  const handleRemoveIngredientRow = (tempId) => {
    setRecipeForm((prev) => ({
      ...
      ingredientes: prev.ingredientes.filter(
        (i) => i.id_temp !== tempId && i.item_codigo !== tempId,
      ),
    }));
  };
  ...
  const handleSaveRecipe = async () => {
    ...
    if (!recipeForm.nombre_version.trim())
      return alert("Por favor, nombra tu receta.");
    if (recipeForm.ingredientes.length === 0)
      return alert("Añade al menos un material a la receta.");
    ...
    try {
      const url = recipeForm.id
        ? `http://localhost:3001/api/ingenierias/${recipeForm.id}`
      ...
      if (res.ok) {
        ...
        setActiveIngenieria(
          dataIng.find((i) => i.es_activa === 1) || dataIng[0] || null,
        );
      }
    } catch (err) {
  ...
  const handleDeleteRecipe = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/ingenierias/${deletingRecipe.id}`,
        { method: "DELETE" },
      );
      if (res.ok) {
  ...
  const handleSyncVentas = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:3001/api/productos-terminados/sincronizar-ventas",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
  ...
  const filteredModalMP =
    mpFilterTerm.trim() === ""
      ? []
      : combinedInventory.filter(
          (item) =>
            item.nombre.toLowerCase().includes(mpFilterTerm.toLowerCase()) ||
            item.codigo.toLowerCase().includes(mpFilterTerm.toLowerCase()),
        );
  const mpTotalPages =
    Math.ceil(filteredModalMP.length / MP_ITEMS_PER_PAGE) || 1;
  const currentPaginatedModalMP = filteredModalMP.slice(
    (mpCurrentPage - 1) * MP_ITEMS_PER_PAGE,
    mpCurrentPage * MP_ITEMS_PER_PAGE,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-[#1E2842] pb-3 shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Configura las recetas y prevé la demanda de stock.
          </p>
        </div>
          ...
          <div className="bg-[#0E1322] border-2 border-[#1E2842] shadow-2xl h-full flex flex-col justify-between overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#1E2842] p-2 bg-[#0B0F19] shrink-0">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-4 py-2 font-pixel text-xs transition-colors border ${mainTab === "SE" ? "bg-[#FF5500]/20 border-[#FF5500] text-[#FF5500]" : "bg-[#161C2E] border-[#1E2842] text-slate-400 hover:text-white"}`}
                >
                  SEMIELABORADOS
                </button>
                <button
                  ...
                  className={`px-4 py-2 font-pixel text-xs transition-colors border ${mainTab === "PT" ? "bg-[#FF5500]/20 border-[#FF5500] text-[#FF5500]" : "bg-[#161C2E] border-[#1E2842] text-slate-400 hover:text-white"}`}
                >
              ...
              {mainTab === "PT" && (
                ...
                >
                  <RotateCw
                    size={12}
                    className={isSyncingVentas ? "animate-spin" : ""}
                  />
                  <span>
                    {isSyncingVentas ? "RECARGANDO..." : "RECARGAR VENTAS"}
                  </span>
                </button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <>
                  ...
                  <table className="w-full text-left text-xs border-collapse table-fixed">
                    ...
                    >
                      <tr className="text-[#00E5FF] font-pixel text-sm">
                        ...
                        <th className="p-3 text-center w-36">Ingeniería</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#1E2842]/60">
                      {currentPaginatedItems.map((item) => (
                        ...
                        >
                          ...
                          {mainTab === "PT" && (
                            <td className="px-3 py-2 border-r border-[#1E2842]/40 text-center font-pixel text-sm font-bold text-[#00E5FF] whitespace-nowrap">
                              {Math.round(item.promedio_ventas_mensual)}
                            </td>
                          )}
                          <td className="px-3 py-2 border-r border-[#1E2842]/40 text-center font-pixel whitespace-nowrap">
                            <span
                              className={`px-2 py-0.5 border text-xs font-bold inline-block ${item.recetas_count > 0 ? "bg-[#00E5FF]/10 border-[#00E5FF]/40 text-[#00E5FF]" : "bg-[#111625] border-[#1E2842] text-slate-500"}`}
                            >
                          ...
                          <td className="px-3 py-2 text-center whitespace-nowrap">
                            <button
                              ...
                              className="px-4 py-1.5 bg-[#111625] border border-[#1E2842] text-white hover:border-[#FF5500] hover:text-[#FF5500] font-pixel text-[10px] transition-all"
                            >
                              VER PLANOS
                            </button>
                          </td>
                ...
                {/* PAGINACIÓN INFERIOR PUESTA EN EL BORDE */}
                <div className="flex items-center justify-between p-3 bg-[#111625] border-t-2 border-[#1E2842] font-pixel text-xs shrink-0">
                  <span className="text-slate-400">
                    Mostrando{" "}
                    <span className="text-white">{indexOfFirstItem + 1}</span> -{" "}
                    <span className="text-white">
                      {Math.min(indexOfLastItem, filteredDataset.length)}
                    </span>{" "}
                    de{" "}
          ...
          <div className="flex flex-col h-full space-y-3 min-h-0">
            <div className="flex items-center gap-3 shrink-0">
              <button
                ...
                className="p-2 bg-[#111625] border border-[#1E2842] text-slate-400 hover:text-white hover:border-white transition-colors"
              >
                <ChevronLeft size={18} />
              </button>
              <h3 className="font-pixel text-lg text-white uppercase truncate flex-1">
                <span className="text-[#FF5500] mr-2">
                  [{selectedItem.codigo}]
                </span>{" "}
                {selectedItem.nombre}
              </h3>
            ...
            <div className="bg-[#0E1322] border-2 border-[#1E2842] shadow-2xl flex flex-col flex-1 min-h-0">
              <div className="p-3 border-b border-[#1E2842] flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#0B0F19] shrink-0">
                <div className="relative w-full sm:w-80" ref={dropdownRef}>
                  <button
                    ...
                    className="w-full flex items-center justify-between bg-black border border-[#1E2842] p-2 text-left hover:border-[#FF5500] transition-colors"
                  >
                    {activeIngenieria ? (
                      <div className="flex items-center gap-2 truncate">
                        {activeIngenieria.es_activa === 1 ? (
                          <Star
                            size={14}
                            className="text-[#FF5500] fill-[#FF5500] shrink-0"
                          />
                        ) : (
                          <div className="w-3.5 shrink-0" />
                        )}
                        <span className="font-pixel text-xs text-white truncate">
                          {activeIngenieria.nombre_version}
                        </span>
                      </div>
                    ) : (
                    ...
                    <div className="absolute top-full left-0 w-full mt-1 bg-black border border-[#1E2842] shadow-2xl z-50">
                      {ingenierias.map((ing) => (
                        <button
                          key={ing.id}
                          onClick={() => {
                            setActiveIngenieria(ing);
                            setIsDropdownOpen(false);
                          }}
                          className="w-full flex items-center justify-between p-2 text-left transition-colors border-b border-[#1E2842]/50 hover:bg-[#111625]"
                        >
                          <div className="flex items-center gap-2 truncate">
                            {ing.es_activa === 1 ? (
                              <Star
                            ...
                            <span className="font-pixel text-[10px] uppercase truncate text-slate-400">
                              {ing.nombre_version}
                            </span>
                          </div>
                        </button>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-[#FF5500] text-black font-pixel font-bold px-4 py-2 border border-white hover:bg-[#FF6600] transition-colors text-xs shrink-0"
                >
              ...
              {ingenierias.length > 0 && activeIngenieria ? (
                  ...
                  <div className="p-3 border-b border-[#1E2842] flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-[#070A12] shrink-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                      {activeIngenieria.es_activa === 0 ? (
                        <button
                          onClick={() => handleSetActiva(activeIngenieria.id)}
                          className="flex items-center gap-1 text-[10px] text-black font-bold font-pixel bg-[#00E5FF] px-2.5 py-1 hover:bg-white transition-colors"
                        >
                          <Check size={12} /> ESTABLECER ACTIVA
                        </button>
                      ) : (
                        <span className="text-[10px] font-pixel text-[#00E5FF] uppercase flex items-center gap-1">
                          <Check size={12} /> RECETA PREDETERMINADA
                        </span>
                      )}
                      {activeIngenieria.updated_at && (
                        <span className="text-[10px] text-slate-400 font-pixel flex items-center gap-1">
                          <Clock size={10} /> ACTUALIZADO:{" "}
                          {formatDate(activeIngenieria.updated_at)}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleOpenEditModal(activeIngenieria)}
                        className="p-1.5 border border-[#1E2842] text-slate-300 hover:text-[#FF5500] hover:border-[#FF5500]"
                      >
                        <Pencil size={13} />
                      </button>
                      <button
                        onClick={() => setDeletingRecipe(activeIngenieria)}
                        className="p-1.5 border border-[#1E2842] text-slate-300 hover:text-red-400 hover:border-red-500"
                      >
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ...
                  <div className="flex-1 overflow-y-auto min-h-0">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead className="sticky top-0 bg-[#161C2E] z-10 border-b border-[#1E2842]">
                        <tr className="text-[#00E5FF] font-pixel">
                          <th className="p-3 border-r border-[#1E2842] font-normal w-24 text-center">
                            Tipo
                          </th>
                          <th className="p-3 border-r border-[#1E2842] font-normal">
                            Componente
                          </th>
                      ...
                      <tbody className="divide-y divide-[#1E2842]/60 border-b-2 border-[#1E2842]">
                        {activeIngenieria.ingredientes.map((ing) => (
                          <tr
                            key={ing.id}
                            className="hover:bg-[#161C2E]/40 text-slate-200 transition-colors"
                          >
                            ...
                            <td className="p-3 border-r border-[#1E2842]/40 text-white">
                              <span className="font-pixel text-[#FF5500] mr-2">
                                [{ing.item_codigo}]
                              </span>{" "}
                              {ing.item_nombre}
                            </td>
                            <td className="p-3 text-right">
                              <span className="font-pixel text-sm text-white font-bold">
                                {ing.cantidad}
                              </span>
                              <span className="text-[10px] text-slate-400 ml-1.5 uppercase font-pixel">
                                {ing.unidad_medida}
                              </span>
                            </td>
                          </tr>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8 opacity-50">
                  ...
                  <span className="font-pixel text-sm text-white mb-1">
            ...
            <div className="p-4 flex items-center justify-between border-b border-[#1E2842] bg-[#0B0F19] shrink-0">
              <div>
                <h3 className="font-pixel text-lg text-[#FF5500]">
                  {recipeForm.id ? "MODIFICAR PLANO" : "NUEVO PLANO DE CRAFTEO"}
                </h3>
            ...
            <div className="p-4 border-b border-[#1E2842] flex flex-col sm:flex-row items-center gap-4 bg-[#070A12] shrink-0">
              <div className="flex-1 w-full">
                <input
                  ...
                  className="w-full bg-[#0E1322] border border-[#1E2842] text-sm text-white p-2 focus:border-[#FF5500] focus:outline-none"
                />
              </div>
              <div className="sm:w-60 w-full">
                <label
                  className={`flex items-center justify-center gap-2 cursor-pointer text-[10px] font-pixel border p-2 transition-colors w-full select-none ${recipeForm.es_activa ? "bg-[#FF5500]/20 border-[#FF5500] text-white font-bold" : "bg-[#0E1322] border-[#1E2842] text-slate-400 hover:text-white"}`}
                >
                  <input
                    type="checkbox"
                    ...
                    className="hidden"
                  />
                  {recipeForm.es_activa && (
                    <Check size={14} className="text-[#FF5500]" />
                  )}
                  <span>
                    {recipeForm.es_activa
            ...
            <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2 min-h-0">
              <div className="border-r border-[#1E2842] flex flex-col min-h-0 bg-[#070A12]">
                <div className="p-3 border-b border-[#1E2842] shrink-0">
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={mpFilterTerm}
                      onChange={handleMpFilterChange}
                      className="w-full bg-[#0E1322] border border-[#1E2842] text-xs text-white pl-8 pr-3 py-1.5 focus:border-[#FF5500] focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-0">
                  {mpFilterTerm.trim() === "" ? (
                    <div className="h-full flex items-center justify-center text-slate-500 font-pixel text-xs text-center">
                      Escribe para buscar
                      <br />
                      MP o Semielaborados.
                    </div>
                  ) : filteredModalMP.length === 0 ? (
                    <div className="p-4 text-center text-slate-500 text-xs font-pixel">
                      Sin resultados.
                    </div>
                  ) : (
                    currentPaginatedModalMP.map((comp) => (
                      <div
                        ...
                        className="flex items-center justify-between p-2 border border-transparent hover:border-[#1E2842] hover:bg-[#111625] transition-colors text-xs"
                      >
                        <div className="truncate pr-2 flex items-center gap-2">
                          <span
                            className={`font-pixel font-bold text-[10px] ${comp.type === "SE" ? "text-amber-400" : "text-[#00E5FF]"}`}
                          >
                            [{comp.type}]
                          </span>
                          <span className="text-[#FF5500] font-pixel">
                            [{comp.codigo}]
                          </span>
                        ...
                        <button
                          onClick={() => handleAddIngredientRow(comp)}
                          className="text-[10px] px-2 py-1 bg-[#111625] border border-[#1E2842] text-white hover:border-[#FF5500] hover:text-[#FF5500] font-pixel transition-colors flex items-center gap-1 shrink-0"
                        >
                ...
                {mpFilterTerm.trim() !== "" && filteredModalMP.length > 0 && (
                  <div className="border-t border-[#1E2842] p-2 flex items-center justify-between font-pixel text-xs bg-[#070A12] shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() =>
                          setMpCurrentPage((p) => Math.max(p - 1, 1))
                        }
                        disabled={mpCurrentPage === 1}
                        className="p-1 bg-[#0E1322] border border-[#1E2842] text-slate-400 hover:text-white disabled:opacity-20"
                      >
                        <ChevronLeft size={14} />
                      </button>
                      <button
                        onClick={() =>
                          setMpCurrentPage((p) => Math.min(p + 1, mpTotalPages))
                        }
              ...
              <div className="flex flex-col min-h-0 bg-[#0E1322]">
                <div className="p-3 border-b border-[#1E2842] bg-[#070A12] shrink-0">
                  <span className="text-[10px] font-pixel text-[#00E5FF] uppercase">
                    COMPONENTES ({recipeForm.ingredientes.length})
                  </span>
                </div>
                <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
                  {recipeForm.ingredientes.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-slate-500 font-pixel text-xs">
                      MESA VACÍA
                    </div>
                  ) : (
                    recipeForm.ingredientes.map((ing) => (
                      <div
                        key={ing.id_temp || ing.item_codigo}
                        className="bg-[#070A12] border border-[#1E2842] p-2 flex items-center justify-between gap-2 hover:border-[#FF5500]/50 transition-colors"
                      >
                        <div className="flex-1 truncate flex items-center gap-2">
                          <span
                            className={`font-pixel font-bold text-[10px] ${ing.item_type === "SE" ? "text-amber-400" : "text-[#00E5FF]"}`}
                          >
                            [{ing.item_type}]
                          </span>
                          <span className="text-white text-xs truncate font-bold">
                            <span className="font-pixel text-[#FF5500] font-normal mr-1">
                              [{ing.item_codigo}]
                            </span>{" "}
                            {ing.item_nombre}
                          </span>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleUpdateIngredientQty(
                                ing.id_temp || ing.item_codigo,
                                e.target.value,
                              )
                            }
                            className="w-16 bg-[#0E1322] border border-[#1E2842] text-[#FF5500] font-pixel text-sm p-1 text-right focus:outline-none focus:border-[#FF5500]"
                          />
                          <span className="text-[9px] text-slate-400 font-pixel w-8 uppercase">
                            {ing.unidad_medida}
                          </span>
                          <button
                            onClick={() =>
